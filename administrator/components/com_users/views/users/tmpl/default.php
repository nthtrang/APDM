	...
	<table class="adminlist" cellpadding="1">
		...
		<?php
			...
			for ($i=0, $n=count( $this->items ); $i < $n; $i++)
			{
				...
				$isAPMD = JAdministrator::isAPDM($row->id);
				if (!$row->block && in_array($row->gid, array(23, 24))) {
					if ($isAPMD){
						$img_apdm = 'tick.png';
						$task_apdm = 'notadpm';
						$title_alt = 'Click to set user cannot access APDM';
					}else{
						$img_apdm = 'publish_x.png';
						$task_apdm = 'apdm';
						$title_alt = 'Click to set user can access APDM';
					}
				}else{
					$img_apdm = '';
					$task_apdm = '';
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<td align="center">
					<?php if ($img_apdm !='') { ?>
					<a href="javascript:void(0);" title="<?php echo $title_alt?>" onclick="return listItemTask('cb<?php echo $i;?>','<?php echo $task_apdm;?>')">
					<img src="images/<?php echo $img_apdm?>" width="16" height="16" border="0" alt="<?php echo $title_alt?>" title="<?php echo $title_alt;?>" /></a>
					<?php } ?>
				</td>
				<td align="center">
