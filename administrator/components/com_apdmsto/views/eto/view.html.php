...
class SToVieweto extends JView
{
    function display($tpl = null)
    {
        ...
        $db->setQuery("select sto.*,wo.wo_code, so.so_cuscode,so.customer_id as ccs_so_code,ccs.ccs_coordinator,ccs.ccs_name,ccs.ccs_code,delivery.* from apdm_pns_sto sto left join apdm_pns_wo wo on sto.sto_wo_id = wo.pns_wo_id left join apdm_pns_so so on so.pns_so_id=sto.sto_so_id  left join apdm_ccs ccs on so.customer_id = ccs.ccs_code left join apdm_pns_sto_delivery delivery on sto.pns_sto_id = delivery.sto_id where sto.pns_sto_id =".$sto_id);
