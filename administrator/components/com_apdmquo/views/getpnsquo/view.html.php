 ...
 * @since 1.0
 */
class QUOViewgetpnsquo extends JView
{
	function display($tpl = null)
	{
        ...
        $db                =& JFactory::getDBO();
        $option             = 'com_apdmquo&task=get_list_pns';
        ...
        if ($so_id){ //get pns_child have exist
           // $query = "SELECT pns_id from apdm_pns_so_fk where so_id =". $so_id;
            if($pns_id)
            {
                ...
                $query = " SELECT pns_id FROM apdm_pns_parents WHERE pns_parent=".$pns_id;
                $db->setQuery($query);
...
//			}
       ...
       if(count($arr_supplier_id) > 0){
        ...
        $where1 = array();
        if (count( $arrPNsChild ) > 0)   {
                $where1[] = 'p.pns_id IN ('.implode(",", $arrPNsChild ).') ';
        }
        ...
        if(isset($arrPNsChild))
        {
            if($where) {
                $where .= ' and p.pns_id IN (' . implode(",", $arrPNsChild) . ') ';
            }
            else
            {
                $where .= ' WHERE p.pns_deleted = 0 and p.pns_id IN (' . implode(",", $arrPNsChild) . ') ';
            }
        }
