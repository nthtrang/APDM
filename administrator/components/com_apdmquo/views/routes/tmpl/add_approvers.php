...
$canApprove = QUOController::canSetRouteQuo($this->quo_row->quotation_id,$this->arr_route[0]->id);
JToolBarHelper::title(JText::_($this->quo_row->quo_code .' '. $this->quo_row->quo_revision ) . $demote . $promote, 'generic.png');
//JToolBarHelper::title( JText::_($this->quo_rowrowEco->eco_name));
$arrayAllowSetRoute = array("Create","Started");
if(in_array($this->arr_route[0]->status,$arrayAllowSetRoute) && $this->arr_route[0]->id != $this->quo_row->quo_routes_id && $canApprove>=2)
{
    JToolBarHelper::customX("set_route_quo", 'unarchive', '', 'Set Route', true);
}
...
<script language="javascript" type="text/javascript">
    function submitbutton(pressbutton) {
        ...
        if (pressbutton == 'summary') {
            window.location.assign("index.php?option=com_apdmquo&task=quo_detail&cid[]=<?php echo $this->quo_row->quotation_id ?>")
            return;
        }
        //		if (pressbutton == 'files') {
        //			  window.location.assign("index.php?option=com_apdmquo&task=files&cid[]=<?php echo $this->quo_row->quotation_id ?>");
        //			return;
        //		}
        if (pressbutton == 'demote') {
            window.location.assign("index.php?option=com_apdmquo&task=demote_quo&cid[]=<?php echo $this->quo_row->quotation_id ?>&routes=<?php echo $cid = JRequest::getVar('routes'); ?>&time=<?php echo time(); ?>");
            return;
        }
        if (pressbutton == 'promote') {
            window.location.assign("index.php?option=com_apdmquo&task=promote_quo&cid[]=<?php echo $this->quo_row->quotation_id ?>&routes=<?php echo $cid = JRequest::getVar('routes'); ?>&time=<?php echo time(); ?>");
            return;
        }
        if (pressbutton == 'affected') {
            window.location.assign("index.php?option=com_apdmquo&task=affected&cid[]=<?php echo $this->quo_row->quotation_id ?>&time=<?php echo time(); ?>");
            return;
        }
        if (pressbutton == 'cancel') {
            window.location.assign("index.php?option=com_apdmquo&task=routes_quo&cid[]=<?php echo $this->quo_row->quotation_id ?>&time=<?php echo time(); ?>");
            return;
        }
        if (pressbutton == 'set_route_quo') {
            window.location.assign("index.php?option=com_apdmquo&task=set_route_quo&cid[]=<?php echo $this->quo_row->quotation_id ?>&time=<?php echo time(); ?>&id=<?php echo $cid = JRequest::getVar('routes'); ?>");
            return;
        }
...
$me = & JFactory::getUser();
$me->get('email');
$owner = $this->quo_row->quo_created_by;
$link = "index.php?option=com_apdmquo&amp;task=saveapproveQuo&amp;cid[]=" . $this->quo_row->quotation_id . "&amp;time=" . time();
if ($owner == $me->get('id')) {
    //       echo $add_routes;
    $link = "index.php?option=com_apdmquo&amp;task=addapproveQuo&amp;cid[]=" . $this->quo_row->quotation_id . "&amp;time=" . time();
}
?>
...
<div class="submenu-box">
    ...
    <div class="m">
        <ul id="submenu" class="configuration">
            ...
            <li><a id="affected" href="index.php?option=com_apdmquo&task=quo_rev&id=<?php echo $this->quo_row->quotation_id;?>"><?php echo JText::_( 'Rev' ); ?></a></li>
            <li><a id="routes"  class="active" href="index.php?option=com_apdmquo&task=routes_quo&cid[]=<?php echo $this->quo_row->quotation_id;?>&time=<?php echo time();?>"><?php echo JText::_( 'Routes' ); ?></a></li>
        </ul>
        <div class="clr"></div>
    </div>
    <div class="b">
...
<form action="<?php echo $link; ?>" method="post" name="adminForm" >
            ...
            foreach ($this->arr_status as $status) {
                ...
                if ($me->get('email') == $status->email || $owner == $me->get('id')) {
                    ...
                    <tr>
                        ...
                        <td align="center" width="15%"><?php echo QUOController::GetNameApproverQuo($status->email); ?>
                        ...
                        <td align="center">
                            <?php if ($status->route_status == "Create" && $owner == $me->get('id') && $status->eco_status=="Inreview") { ?>
                                <a href='index.php?option=com_apdmquo&task=removeapprove&cid[]=<?php echo $this->quo_row->quotation_id; ?>&id=<?php echo $status->id; ?>&time=<?php echo time(); ?>&routes=<?php echo JRequest::getVar('routes') ?>'>Remove</a>
                            <?php } ?>
                        </td>
                    </tr>
                    <?php
                    ...
                    <tr>
                        ...
                        <td align="center"><?php echo QUOController::GetNameApproverQuo($status->email); ?>
                            <input type="hidden" name="mail_user[]" id="title" value="<?php echo $status->email; ?>" />
                        </td>
                        <?php
                        if ($status->eco_status == 'Inreview') {
                ...
                <tr id="<?php echo $j; ?>" style="display:block">
                    ...
                    if ($status->eco_status == 'Inreview') {
                        ...
                        <td  align="center" width="7%">
                            ...
                            ?>
                            <a href='index.php?option=com_apdmquo&task=approve&cid[]=<?php echo $this->quo_row->quotation_id; ?>&time=<?php echo time(); ?>'></a>
                        </td>
    ...
    <input type="hidden" name="eco_id" value="<?php echo $this->quo_row->quotation_id ?>" />
    <input type="hidden" name="cid[]" value="<?php echo $this->quo_row->quotation_id ?>" />
