...
//$fp = fopen ($path_pns."file.txt", "w"); 
        ...
        function ajax_add_pns_eto() {
                ...
                foreach($fk_ids as $fk_id)
                {
                        ...
                        $db->setQuery("INSERT INTO apdm_pns_sto_fk (pns_id,sto_id,partstate,pns_mfg_pn_id) VALUES ( '" . $pns_id . "','" . $sto_id . "','','0')");
                        $db->query();                         
                }                 
...
//                $db->query();    
 ...
 /**
    ...
    function GetLocationFromPartStatePns_bk($partState,$pns_id)
    {
        $db = & JFactory::getDBO();
        $rows = array();
        //$query = "SELECT fk.id  FROM apdm_pns_sto AS sto inner JOIN apdm_pns_sto_fk fk on sto.pns_sto_id = fk.sto_id inner join apdm_pns AS p on p.pns_id = fk.pns_id where fk.pns_id=".$pns_id;
        ...
        function getSofomId($so_id)
{
    ...
    function GetTotalEtoPns($pns_id,$so_id)
    {
        $db =& JFactory::getDBO();
        $query = "SELECT count(DISTINCT(fk.sto_id)) FROM apdm_pns_sto_fk AS fk inner JOIN apdm_pns_sto sto on sto.pns_sto_id = fk.sto_id inner join apdm_pns AS p on p.pns_id = fk.pns_id left join apdm_pns_so so on sto.sto_so_id = so.pns_so_id  where  so.pns_so_id=  ".$so_id." and fk.pns_id = ".$pns_id." and sto.sto_isdelivery_good = 1 and sto.sto_type = 2 and sto.sto_state in ('InTransit','Done') GROUP by fk.sto_id";
        $db->setQuery($query);
        return $db->loadResult();
...
//var_dump($objPHPExcel->getActiveSheet()->toArray(null,true,true,true));
       ...
       function shopfloor()
        ...
        function save_pause_step()
        {                                          
                JRequest::setVar('layout', 'popup_pause_step');
                JRequest::setVar('view', 'wo');
                parent::display();                
        }
        function save_resume_step()
        {                                          
                JRequest::setVar('layout', 'popup_resume_step');
                JRequest::setVar('view', 'wo');
                parent::display();                
        }
...
//                        $db->query(); 
        ...
        {                
                ...
                $pass_step = 1;
                if(!$post['op_final_value1']||!$post['op_final_value2']||!$post['op_final_value3']||!$post['op_final_value4']||!$post['op_final_value5']||!$post['op_final_value6']||!$post['op_final_value7']||!$post['op_final_value8'])
                {
                        $pass_step = 0;
                }
...
//                $db->query(); 
        ...
        function saveReworkStepWo()
        {
                ...
                $wo_step = JRequest::getVar('wo_step');
                $op_comment = JRequest::getVar('op_comment');
                $qc_by = JRequest::getVar('qc_by');
                $rework_failure = JRequest::getVar('rework_failure');
                $rework_qty = JRequest::getVar('rework_qty');
                $rework_times = JRequest::getVar('rework_times');
                $rework_from  = JRequest::getVar('step_rework');
                $username = JRequest::getVar('username', '', 'method', 'username');
                $password = JRequest::getVar('passwd', '', 'post', 'string', JREQUEST_ALLOWRAW);
                $query = "select user_id from apdm_users where user_password = md5('".$password."') and username='".$username."'";
                $db->setQuery($query);
    ...
    function wo_rework_log()
    {
            JRequest::setVar('layout', 'wo_rework_list');
                JRequest::setVar('view', 'wo');                
                parent::display();   
    }
     function download_file_rework() {
                $db = & JFactory::getDBO();
                $file = JRequest::getVar('file');
                $wo_id = JRequest::getVar('id');                
                $path_so = JPATH_SITE . DS . 'uploads' . DS . 'wo' . DS;
                $path_cads = $path_so  . DS . $wo_id . DS;                                          
                $dFile = new DownloadFile($path_cads, $file);
                exit;
        }     
