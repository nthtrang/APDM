...
function UpdatePnsChild(){
	if ($('boxchecked').value==0){
		...
		var url = 'index.php?option=com_apdmpns&task=ajax_list_pns_so';			
		var MyAjax = new Ajax(url, {
			method:'post',
			data:  $('adminFormPns').toQueryString(),
			onComplete:function(result){				
				window.parent.document.getElementById('pns_child_so').innerHTML = result;                                
...
<form action="index.php?option=com_apdmpns&task=get_list_pns_so&tmpl=component" method="post" name="adminForm" id="adminFormPns"  >
