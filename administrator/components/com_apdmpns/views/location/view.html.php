 ...
 * @since 1.0
 */
class pnsViewlocation extends JView
{
	function display($tpl = null)
	{
	   ...
	   // global $mainframe, $option;
        global $mainframe, $option;
        $option             = 'com_apdmpns_location';
        $db                =& JFactory::getDBO();
        $cid		= JRequest::getVar( 'cid', array(0), '', 'array' );       
        $location_id		= JRequest::getVar( 'id');       
        $me 		= JFactory::getUser();
        ...
        if (isset( $search ) && $search!= '')
        {
            ...
            $where[] = 'p.location_code LIKE '.$searchEscaped.' or p.location_description LIKE '.$searchEscaped.'';        
           
        }  
      
        $where = ( count( $where ) ? ' WHERE (' . implode( ') AND (', $where ) . ')' : '' );
        $orderby = ' ORDER BY p.pns_location_id desc';        
        $query = 'SELECT COUNT(p.pns_location_id)'
        . ' FROM apdm_pns_location AS p'
        . $where
        ;
...
//         $this->assignRef('pos_list',        $pos_list);     
        //for PO detailid
         $db->setQuery("SELECT po.*,fk.id,fk.qty, p.pns_description,p.pns_cpn,p.pns_id,p.pns_stock,CONCAT_WS( '-', p.ccs_code, p.pns_code, p.pns_revision ) AS parent_pns_code  FROM apdm_pns_po AS po inner JOIN apdm_pns_po_fk fk on po.pns_po_id = fk.po_id inner join apdm_pns AS p on p.pns_id = fk.pns_id where po.pns_po_id=".$location_id);
         $pns_list = $db->loadObjectList();         
         $this->assignRef('po_pn_list',        $pns_list);     
         
         $db->setQuery("SELECT * from apdm_pns_location where pns_location_id=".$location_id);
         $location_row =  $db->loadObject();
         $this->assignRef('location_row',        $location_row);
        $lists['search']= $search;    
        $this->assignRef('lists',        $lists);
        $this->assignRef('location_list',        $rows);
