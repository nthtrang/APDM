	...
	<fieldset class="adminform">
		...
		<table class="adminlist" cellspacing="1" width="400">
				...
				<tbody>					
					...
					<tr><td><input type="hidden" name="m_exist[]" value="<?php echo $m['id']?>" ><input type="hidden" name="m_exist_id[]" value="<?php echo $m['m_id']?>" ><?php echo $m['m_name']?> </td><td><input type="text" size="40" value="<?php echo $m['m_value'];?>" name="m_exist_value[]" /> </td><td><a href="index.php?option=com_apdmpns&task=remove_infomf&id=<?php echo $m['id']?>&pns_id=<?php echo $this->row->pns_id?>" title="Click to remove"><?php echo JText::_('Remove')?></a></td></tr>
		<?php }
		 } ?>
				</tbody>
				</table>		
