...
class ecoViewlistpns extends JView
{
	...
	$cid		= JRequest::getVar( 'cid', array(0), '', 'array' );
        $query = "SELECT p.*, CONCAT_WS( '-', p.ccs_code, p.pns_code, p.pns_revision ) AS pns_code_full FROM apdm_pns AS p LEFT JOIN apdm_pns_parents as pr ON p.pns_id=pr.pns_id LEFT JOIN apdm_ccs AS c ON c.ccs_code = p.ccs_code LEFT JOIN apdm_eco AS e ON e.eco_id=p.eco_id  WHERE  c.ccs_activate= 1 AND c.ccs_deleted=0 AND  p.pns_deleted =0 AND p.eco_id=".$cid[0]." and p.pns_life_cycle ='Create'  group by pns_id ORDER BY p.ccs_code";        
        $db->setQuery( $query);
