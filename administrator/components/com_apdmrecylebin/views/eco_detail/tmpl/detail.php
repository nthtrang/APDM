...
<?php
	...
	JToolBarHelper::cancel( 'cancel_detaileco', 'Close' );
...
<script language="javascript" type="text/javascript">
	function submitbutton(pressbutton) {
		...
		if (pressbutton == 'cancel_detaileco') {
		...
		<legend><?php echo JText::_( 'INFORMATION_DETAIL' ); ?></legend>
			<table class="admintable" cellspacing="1">				
				...
				<tr>
					...
					<td>
						<?php echo ($this->row->eco_status) ? 'Activated' : 'Inactivated';  ?>
					</td>
				</tr>	
				<tr>
					<td class="key" valign="top">
					...
					<td>
						<?php echo $this->row->eco_activate;  ?>
					</td>
				</tr>				
			...
			<table class="admintable">
				<?php if (count($this->arr_file) > 0 ) { ?>
					...
					foreach ($this->arr_file as $file) { 
						$filesize = RecyleBinController::Readfilesizeeco($file->file_name);
					?>
							<tr>
							<td><?php echo $i?></td>
