 ...
 * @since 1.0
 */
class recylebinVieweco extends JView
{
	function display($tpl = null)
	{
		...
		$db				=& JFactory::getDBO();
		$option   = "com_apdmrecylebin&task=eco";
		...
		$where = array();
		$where[] = 'e.eco_deleted = 1';
		if (isset( $search ) && $search!= '')
		{
			$searchEscaped = $db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
			$where[] = 'e.eco_name LIKE '.$searchEscaped.' OR e.eco_pdf LIKE '.$searchEscaped.' OR e.eco_description LIKE '.$searchEscaped;
		}			
