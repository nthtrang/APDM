...
/**
    ...
    function getToolTrackerScan()
    {
            $db = & JFactory::getDBO();
            $tool_code = JRequest::getVar('tool_code');
            $query = "select tto.pns_tto_id from apdm_pns_tto tto inner join apdm_pns_tto_fk fk on tto.pns_tto_id = fk.tto_id inner join apdm_pns_location loc on fk.location = loc.pns_location_id where loc.location_code = '".$tool_code."' and tto.tto_state = 'Using' order by tto.pns_tto_id desc limit 1";
            $db->setQuery($query);
            $pns_tto_id =  $db->loadResult();             
                if ($pns_tto_id) {
                        return $this->setRedirect('index.php?option=com_apdmtto&task=tto_detail&id='.$pns_tto_id);
                }
