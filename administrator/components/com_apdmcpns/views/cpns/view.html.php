...
class cpnsViewcpns extends JView
{
            ...
            switch($type_filter){
                ...
                $leght = strlen (trim($keyword));                    
                 if ($leght==16){                                                                        
                       ...
                       $db->setQuery("SELECT pns_id FROM apdm_pns WHERE pns_cpn = 1 and ccs_code=".$arr_code[0]." AND pns_code='".$arr_code[1].'-'.$arr_code[2]."' AND pns_revision='".$arr_code[3]."'");
                       ...
                       $arr_code = explode("-", trim($keyword));                         
                       $db->setQuery("SELECT pns_id FROM apdm_pns WHERE  pns_cpn = 1 and  ccs_code=".$arr_code[0]." AND pns_code='".$arr_code[1].'-'.$arr_code[2]."'");
                       $rs_pns = $db->loadObjectList();                       
                       if (count($rs_pns) > 1){
