...
<?php
	...
	JToolBarHelper::title( JText::_( 'CPNS_MAMANGEMENT' ) , 'cpanel.png' );
	if (in_array("E", $role)) {
...
<form action="index.php?option=com_apdmcpns" method="post" name="adminForm" onsubmit="submitbutton('')" >
...
<!--				<th width="20%" class="title">
		...
		<?php
			...
			{
				$row 	=& $this->rows[$i];
				$link 	= 'index.php?option=com_apdmcpns&amp;task=detail&cid[0]='.$row->pns_id;	
				...
				//echo $pns_image;
				$mf = CPNsController::GetManufacture($row->pns_id);
				$bom = CPNsController::GetChildParentNumber($row->pns_id);
                                $wheruse = CPNsController::GetChildWhereNumber($row->pns_id);
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<?php if ($bom) {                                       
                                ?>
				<a href="index.php?option=com_apdmcpns&task=bom&id=<?php echo $row->pns_id; ?>" title="<?php echo JText::_('LINK_PART_HIERARCHY')?>" >
				...
				<?php } ?>
                                ...
                                <td>
				<?php if ($wheruse) { ?>
				<a href="index.php?option=com_apdmcpns&task=whereused&id=<?php echo $row->pns_id; ?>" title="<?php echo JText::_('LINK_PART_HIERARCHY')?>" >
					<img src="images/search_f2.png" width="16" height="16" border="0" title="<?php echo JText::_('LINK_PART_HIERARCHY')?>" alt="<?php echo JText::_('LINK_PART_HIERARCHY')?>" /></a>
				<?php } else {?>
					<img src="images/search.png" width="16" height="16" border="0" title="<?php echo JText::_('NO_PART_HIERARCHY')?>" alt="<?php echo JText::_('NO_PART_HIERARCHY')?>" />
				<?php } ?>
				...
				<td align="center">
					<?php echo CPNsController::GetECO($row->eco_id); ?>
				</td>
				<td>
					<?php if($row->pns_pdf !="") { ?>
					 <a href="index.php?option=com_apdmcpns&task=download&id=<?php echo $row->pns_id?>" title="<?php echo JText::_('CLICK_HERE_TO_DOWNLOAD_FILE_PDF')?>"><img src="images/downloads_f2.png" width="16" height="16" border="0" alt="<?php echo JText::_('CLICK_HERE_TO_DOWNLOAD_FILE_PDF')?>" /></a>
					<?php }else{ ?>
