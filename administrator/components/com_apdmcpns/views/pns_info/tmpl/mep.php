...
<div class="submenu-box">
		...
		<ul id="submenu" class="configuration">
			...
			<li><a id="whereused" href="index.php?option=com_apdmcpns&task=whereused&id=<?php echo $this->row->pns_id;?>"><?php echo JText::_( 'Where Used' ); ?></a></li>
                        <li><a id="specification" href="index.php?option=com_apdmcpns&task=specification&cid[]=<?php echo $this->row->pns_id;?>"><?php echo JText::_( 'Specification' ); ?></a></li>
...
<form action="index.php" method="post" name="adminForm" enctype="multipart/form-data" >
	<div class="col width-60">
		...
		?>
		 ...
		 ?>			<tr><td><input type="hidden" name="v_exist[]" value="<?php echo $v['id']?>" ><input type="hidden" name="v_exist_id[]" value="<?php echo $v['v_id']?>" ><?php echo $v['v_name']?> </td><td><input size="40" type="text" value="<?php echo $v['v_value'];?>" name="v_exist_value[]" /> </td><td><a href="index.php?option=com_apdmcpns&task=remove_info&id=<?php echo $v['id']?>&pns_id=<?php echo $this->row->pns_id?>" title="Click to remove"><?php echo JText::_('Remove')?></a></td></tr>
		<?php }
		  ?>	
			</tbody>
			</table>
...
<input type="button" name="addVendor" value="<?php echo JText::_('Select Vendor')?>"/>
	...
	<fieldset class="adminform">
		...
		<?php if (count($this->lists['arr_s']) > 0) { ?>
		 ...
		 ?>			<tr><td><input type="hidden" name="s_exist[]" value="<?php echo $s['id']?>" ><input type="hidden" name="s_exist_id[]" value="<?php echo $s['s_id']?>" ><?php echo $s['s_name']?> </td><td><input type="text" size="40" value="<?php echo $s['s_value'];?>" name="s_exist_value[]" /></td><td><a href="index.php?option=com_apdmcpns&task=remove_info&id=<?php echo $s['id']?>&pns_id=<?php echo $this->row->pns_id?>" title="Click to remove"><?php echo JText::_('Remove')?></a></td></tr>
		<?php }
		 ?>				
				</tbody>
				</table>
	...
	<fieldset class="adminform">
		...
		<table class="adminlist" cellspacing="1" width="400">
				...
				<tbody>					
					...
					<tr><td><input type="hidden" name="m_exist[]" value="<?php echo $m['id']?>" ><input type="hidden" name="m_exist_id[]" value="<?php echo $m['m_id']?>" ><?php echo $m['m_name']?> </td><td><input type="text" size="40" value="<?php echo $m['m_value'];?>" name="m_exist_value[]" /> </td><td><a href="index.php?option=com_apdmcpns&task=remove_infomf&id=<?php echo $m['id']?>&pns_id=<?php echo $this->row->pns_id?>" title="Click to remove"><?php echo JText::_('Remove')?></a></td></tr>
		<?php }
		 } ?>
				</tbody>
				</table>		
