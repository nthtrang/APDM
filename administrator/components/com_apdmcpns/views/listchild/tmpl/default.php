...
function DeletePnsChild(){

	if ($('boxchecked').value==0){
		alert('Please select PNs to delete');
		return false;
	}else{	
		document.getElementById('task').value='remove_parent';
		var url = 'index.php?option=com_apdmpns&task=remove_parent';			
		var MyAjax = new Ajax(url, {
			method:'post',
			data:  $('adminFormPns').toQueryString(),
			onComplete:function(result){				
				window.document.getElementById('list_pns_child').innerHTML = result;								
			}
		}).request();
