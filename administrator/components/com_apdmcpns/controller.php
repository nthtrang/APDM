...
class CPNsController extends JController
{
	/**
	 * Constructor
	 *
	...
	*/
    function rev_roll(){
       ...
       // $arrLast    = array('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'K', 'L', 'M', 'N', 'P', 'R', 'T', 'V', 'Y', 'Z');
        ...
        $new_code  = CPNsController::RandomRevision();             
       ...
       // $db->setQuery("SELECT ccs_code FROM apdm_ccs WHERE ccs_id=".$row->ccs_id);
		...
		switch ( $this->getTask() )
		{
			case 'apply':
				...
				$this->setRedirect( 'index.php?option=com_apdmcpns&task=detail&cid[0]='.$row->pns_id, $msg );
				break;
			...
			default:
				...
				$this->setRedirect( 'index.php?option=com_apdmcpns&task=add', $msg );
				break;
		}
	}
   ...
   */
	...
	*/
    function download_cad_all_pns(){
        ...
        $zipsize = CPNsController::size_format($zipsize);
                ...
                foreach ($rs as $obj){
                 ...
                 //   echo  $querypn;
                   ...
                   // echo $type;
                    ...
                    $resutl .= CPNsController::DisplayPnsChild($obj->pns_id, $vcd);
                ...
                foreach ($rs as $obj){
                 ...
                 //   echo  $querypn;
                   ...
                   // echo $type;
                    ...
                    $result .= $obj->pns_id.',';
                    $result .= CPNsController::DisplayPnsChildId($obj->pns_id, $vcd);            
                }
...
//$fp = fopen ($path_pns."file.txt", "w"); 
          foreach ($list_cads as $cad){
              ...
              $filesize = CPNsController::Readfilesize('cads', $cad->cad_file, $row->ccs_code, $folder_pns);    
	...
	function GetArrayPNsChild($pns){
		...
		// $query 	= "SELECT * FROM apdm_pns_parents WHERE pns_parent=".$pns.' ORDER BY ccs_code ';	
		 ...
		 if ( count ($rows) > 0 ){
		 	foreach ($rows as $row){
				$arrChild[] = $row->pns_id;
				$arrChild[]= CPNsController::GetArrayPNsChild($row->pns_id);
			}
		 }
		 return $arrChild;
	}
    function GetListPnsParent($pns){
        ...
        if (count($rs) > 0){
            foreach ($rs as $obj){
                $result .= $obj->pns_id.', ';        
                $result .= CPNsController::GetListPnsParent($obj->pns_id);             
