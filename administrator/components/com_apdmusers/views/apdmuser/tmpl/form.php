...
<script language="javascript" type="text/javascript">
	function submitbutton(pressbutton) {
		...
		if (trim(form.first_name.value) == "") {
			alert( "<?php echo JText::_( 'Please input full name', true ); ?>" );
		} else if (form.username.value == "") {
			alert( "<?php echo JText::_( 'You must input username', true ); ?>" );
		} else if (r.exec(form.username.value) || form.username.value.length < 6) {
			alert( "<?php echo JText::_( 'username must than 6 characters', true ); ?>" );		
			...
			alert( "<?php echo JText::_( 'You must input email.', true ); ?>" );
		}else if (!isEmail(form.email.value)) {
			alert( "<?php echo JText::_( 'Email not correct.', true ); ?>" );	
		} else if (form.gid.value == "23" && form.role_user.value=="") {
			alert( "<?php echo JText::_( 'You must provide role of user', true ); ?>" ); 
		} else {
			submitform( pressbutton );
		}
	}
...
<form action="index.php" method="post" name="adminForm" autocomplete="off">
		...
		<legend><?php echo JText::_( 'User Details' ); ?></legend>
			<table class="admintable" cellspacing="1">
				...
				<tr>
					...
					<td>
						<input type="text" name="username" id="username" class="inputbox" size="40" value="<?php echo $this->user[0]->username; ?>" autocomplete="off" <?php if( $this->user[0]->id) { ?> readonly="readonly" <?php } ?>   />
					</td>
				</tr>
				<?php if (trim($this->user[0]->password) !='' && $this->user[0]->id || !$this->user[0]->id) { ?>
				<tr>
					<td class="key">
						<label for="password">
				...
				<?php }else{ ?>
				<input class="inputbox" type="hidden" name="password" id="password" size="40" value=""/>
				<input class="inputbox" type="hidden" name="password2" id="password2" size="40" value=""/>
				<?php } ?>
				<tr>
					<td class="key">
						<label for="password2">
					...
					<td>
							...
							<input class="inputbox" type="text" name="email" id="email" size="40" value="<?php echo $this->user[0]->email;?>"/>
	...
	<div class="clr"></div>
	<input type="hidden" value="0" id="block0" name="block"/>
