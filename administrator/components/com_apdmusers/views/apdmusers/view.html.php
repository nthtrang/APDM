<?php
/**
* HTML View class for the Apdm user list.
* @package		APDM
...
class APDMUsersViewAPDMUsers extends JView
{
	function display($tpl = null)
	{
		...
		$query = 'SELECT a.*, u.name, u.id, u.lastvisitDate '
				. ' FROM apdm_users AS a'
		...
		$query_role = "SELECT ur.role_id as value, r.role_name as text
						...
						INNER JOIN apdm_role AS r ON r.role_id = ur.role_id
						GROUP BY value
						ORDER BY value
						";
		...
		$db->setQuery("SELECT a.user_create_by as value, u.name as text FROM apdm_users as a LEFT JOIN jos_users as u ON u.id=a.user_create_by WHERE u.block=0 GROUP BY a.user_create_by ");
