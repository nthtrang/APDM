...
class UserController extends JController
{
	...
	function login()
	{
        ...
        //check user login first time or not
           /*     $db        =& JFactory::getDBO();
                $query_check = "SELECT * FROM jos_users WHERE email='".$credentials['username']."' ";     
                ...
                if ($usertype =='Registered'){ //send mail infrom user
                    $MailFrom     = $mainframe->getCfg('mailfrom');
                    $FromName     = $mainframe->getCfg('fromname');

                // Prepare email body
                    
                    $body     = JText::_('BODY_EMAIL_WITH_FIRST_TIME_RIGISTRATION');
                    $subject = JText::_('SUBJECT_INFORM_ADMINISTRATOR_CONFIRM_ACCOUNT');
                    $mail = JFactory::getMailer();

                    $mail->addRecipient( $MailFrom );
                    $mail->setSender( array( $credentials['username'], $credentials['username'] ) );
		...
		if(!JError::isError($error))
		{
			...
			$db		=& JFactory::getDBO();
			$query_check = "SELECT * FROM jos_users WHERE username='".$credentials['username']."' ";	
			...
			//	$FromName 	= $mainframe->getCfg('fromname');    
			  ...
			  //$body 	= JText::_('This e-mail is already registered. If you forgot your password, PLEASE CONTACT IT ADMINISTRATOR');
			//	$subject = JText::_('SUBJECT_INFORM_USER_ACCOUNT');
                
            //  JUtility::sendMail( $MailFrom, $FromName, $credentials['username'], $subject, $body, 1 ); 
				 
			
			// Inform administrator
			if (!$_SESSION['email_autoregister'] )   { 
				
					$MailAdmin     = $mainframe->getCfg('mailfrom');    
					$date =& JFactory::getDate();
					$subject = JText::_('REGMAIL_SUBJECT') . ' ' . $credentials['username'] . ' - REMIND'; 
					$message = 'New account request:<br>' . '+ User: ' . $credentials['username'] . '<br>+ Time of register: ' . $date->toRFC822();
                    $email_user = $row->email;
					JUtility::sendMail( $email_user, $email_user, $MailAdmin, $subject, $message, 1 );   
				}
				
			}else{				
				$msg = JText::_('WARNREG_EMAIL_INUSE');
			}
